...
- Сообщение пользователя о том, что функция не работает, считать важной
  гипотезой, но не доказанным фактом.
- До вывода о поломке выполнить доступную read-only проверку фактического
  состояния: воспроизвести сценарий, изучить актуальные RunEvents/логи/статус и
  проверить наблюдаемый критерий успеха.
- Если проверка показывает, что функция работает, начинать ответ с этого факта,
  приводить конкретное подтверждение и давать пользователю короткий точный
  сценарий самостоятельной проверки.
- Если проверить работоспособность невозможно, говорить «не удалось проверить»
  или «состояние не подтверждено», а не утверждать, что функция сломана.
- Различать реальный дефект, устаревший UI, неверно выбранный Project/Run,
  незавершённую асинхронную операцию и ошибочное ожидание пользователя.
